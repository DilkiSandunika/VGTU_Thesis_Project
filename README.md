...
## Examples of (X, y) for Functional Requirement Refinement

![X → y Transformation for Functional Requirement Refinement](X_Y.png)

This directory contains concrete examples of the input-output relationship (`X` -> `y`) that defines the core task of this thesis. The goal is to transform a raw, semi-structured requirement (`X`) into a refined, compliant, and well-formed requirement (`y`) using a Retrieval-Augmented Generation (RAG) pipeline.

### Definition of (X, y)

*   **`X` (The Input / Problem):** A raw functional requirement as extracted from a source document. These inputs may be informally worded, lack a standard structure, or be non-compliant with project standards.

*   **`y` (The Output / Solution):** The refined functional requirement after being processed by the RAG pipeline. The ideal output `y` is structured, compliant with the rules in the knowledge base, and written in a clear, active voice.

The transformation from `X` to `y` is performed by the end-to-end solution, which uses a knowledge base and the Google Gemma LLM to perform the refinement.

### Examples

The file `X_y_examples.csv` contains a set of 10 examples generated by running the full pipeline (`notebooks/03_end_to_end_pipeline_demo.ipynb`).

#### Example 1: Authentication Requirement Transformation
* `X (Input):`  "Users can login with username and password"
* `Y (Output):` "The system SHALL provide a secure authentication mechanism that enables registered users to access the application by entering valid username and password credentials. The system SHALL validate credentials against the user database and grant access upon successful authentication."
* `Compliance Score:` 0.91
* `Retrieved Rules:` RULE-001 (Modal Verbs), RULE-005 (Performance)

#### Example 2: Search Functionality Transformation
* `X (Input):`  "Allow searching of crime records"
* `Y (Output):` "The system SHALL provide advanced search functionality enabling authorized users to query crime records using multiple search criteria including case number, date range, location, and crime type. Search results SHALL be displayed within 3 seconds and support pagination with export capabilities."
* `Compliance Score:` 0.91
* `Retrieved Rules:` RULE-001 (Modal Verbs), RULE-005 (Performance)

#### Example 3: Data Processing Transformation
* `X (Input):`  "System processes reports and generates analytics"
* `Y (Output):` "The system SHALL automatically process submitted crime reports and generate comprehensive analytical outputs including statistical trends, geographic heatmaps, and predictive insights. Processing SHALL complete within 5 minutes of report submission, and generated analytics SHALL be accessible through the dashboard interface."
* `Compliance Score:` 0.89
* `Retrieved Rules:` RULE-002 (Acceptance Criteria), RULE-005 (Performance)

#### Columns in `X_y_examples.csv`:

*   **`X_Input_Requirement`**: This column represents **X**. It contains the original, un-processed requirement text.
*   **`y_Output_Requirement`**: This column represents **y**. It contains the final, refined requirement generated by the Gemma model.
*   **`Compliance_Score`**: An automated score that validates the quality of `y` against the retrieved rules.
*   **`Retrieved_Context_Rules`**: The specific rules from the knowledge base that were retrieved and used as context to transform `X` into `y`.

These examples serve as a concrete demonstration of the thesis's proof-of-concept.
